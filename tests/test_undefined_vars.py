...
def test_undefined_variable_silent_in_normal_mode(capsys) -> None:
    """Undefined variables should be silent in normal mode."""
...
def test_undefined_variable_warns_in_strict_mode(capsys) -> None:
    """Undefined variables should warn in strict mode."""
    ctx = BuildContext(source_dir=Path("."), build_dir=Path("build"))
    commands = [
        Command(name="set", args=["RESULT", "${UNDEFINED_VAR}"], line=1),
    ]
...
def test_nested_undefined_variable_warns_in_strict_mode(capsys) -> None:
    """${${VAR}} should warn in strict mode when unresolved."""
    ctx = BuildContext(source_dir=Path("."), build_dir=Path("build"))
    commands = [
