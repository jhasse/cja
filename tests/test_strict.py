...
def test_include_cmake_parse_arguments_strict() -> None:
    """Test that include(CMakeParseArguments) works in strict mode."""
    ctx = BuildContext(source_dir=Path("."), build_dir=Path("build"))
    commands = [
        Command(name="include", args=["CMakeParseArguments"], line=1),
    ]
    # Should not raise
    process_commands(commands, ctx, strict=True)
